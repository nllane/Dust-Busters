...
void Estop() {
  ...
  run_motor(B, 0);
  test=1;
  digitalWrite(vacumme, LOW);// turn off vacumme
  ...
  digitalWrite(vacumme, HIGH); //turn vacumme on
}
...
//inconsistant recive something is wrong
  ...
  void checkSensers(){
float AvgAcs=0,AcsValue=0.0,Samples=0.0,vacCurrent=0.0,brushCurrent=0.0,Volt=0.0,VacummeVolt=0.0,RCurrent=0.0,LCurrent=0.0,AcsValueF=0.0;
Serial.print("Check");
delay(2000);
...
if (brushCurrent >5){
  ...
  digitalWrite(brush, LOW);
  digitalWrite(vacumme, LOW);
    run_motor(A, 0);
    run_motor(B, 0);
    digitalWrite(Blue, HIGH);
    while(digitalRead(button) == HIGH);
    digitalWrite(Blue, LOW);}
if (RCurrent>8||LCurrent>8){
